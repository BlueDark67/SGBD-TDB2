...
if (is_user_logged_in()) {
    if(current_user_can('insert_values')) {
        ...
        if ($estado === 'procurar_crianca') {
            ...
            echo '<strong>Data de Nascimento:</strong> <input type="text" name="data_nascimento" placeholder="AAAA-MM-DD"><br>';
            echo '<input type="hidden" name="estado" value="escolher_crianca">';  // Define o estado seguinte
            echo '<br>';
            echo '<input id="btn" type="submit" value="Submeter">';
            echo '<br>';
            echo '<br>';
            echo '</form>';
            ...
            if (isset($_REQUEST['crianca'])) {
                ...
                if ($result_item_types) {
                    ...
                    while ($row_item_type = mysqli_fetch_assoc($result_item_types)) {
                        ...
                        echo "<li><strong>" . htmlspecialchars($category_name) . "</strong></li>";
    ...
    ";
        ...
        ";
                ...
                if ($result_subitens) {
                    while ($subitem = mysqli_fetch_assoc($result_subitens)) {
                        ...
                        echo "<label for='$field_name'><strong>$subitem_name</strong><span class='vermelho'>*</span></label><br>";
              ...
              <label for='{$field_name}_no'>Não</label>";
                ...
                // Botão de submissão
                echo "<button id='button' type='submit'>Submeter</button>";
                echo "</form>";
            } else {
        ...
        elseif ($estado === 'validar') {
            ...
            if (!empty($missing_fields)) {
                ...
                echo "<p><strong>Confirma que os dados estão corretos e pretende submeter os mesmos?</strong></p>";

                echo "<form method='post' action='{$current_page}?estado=inserir&item=$item_id'>";
                foreach ($submitted_data as $key => $value) {
                    echo "<input type='hidden' name='$key' value='$value'>";
                }
                echo "<button id='btn' type='submit'>Submeter</button>";
                echo "</form>";
            }
            echo '<br>';
            goBackLink();
        ...
        elseif ($estado === 'inserir') {
            ...
            $producer = $current_user->user_login; // Usuário
            ...
            if ($result_subitens) {
                ...
                $required_fields_missing = false; // Para verificar campos obrigatórios não preenchidos
                ...
                while ($subitem = mysqli_fetch_assoc($result_subitens)) {
                    ...
                    if (isset($_POST[$field_name])) {
                        $value = mysqli_real_escape_string($conn, $_POST[$field_name]);

                        // Verifica se o valor não está vazio antes de inserir
                        if (!empty($value)) {
                            // Consulta SQL para inserir os valores na tabela `value`
                            $sql_insert = "
            ...
            echo "<p><strong> Inseriu o(s) valor(es) com sucesso</strong></p>";
            echo "<p><strong>Clique em Voltar para voltar ao início da inserção de valores ou em Escolher item se quiser continuar a inserir valores associados a esta criança</strong></p>";
